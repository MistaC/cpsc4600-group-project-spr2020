...
def charIndex(char):
    return string.ascii_lowercase.index(char) #ORIGINAL: string.printable
def indexChar(index):
    return string.ascii_lowercase[index] #ORIGINAL: string.printable
...
def genKey():
    ...
    while True:
        try:
            for row in range(3):
                for col in range(3):
                    key_matrix[row][col] = 0+secrets.randbelow(26) #ORIGINAL: 100
            inverse_key = Matrix(key_matrix).inv_mod(26) #ORIGINAL: 100
...
def encrypt(key,msg): #key is key matrix. msg is plaintext string.
    ...
    for c in msg: #Loop three times. Get a character ascii value each time. Do matrix mult at 3. Convert resulting values into letters and concat with text variable.
        if iter > 2: #3 ascii values already in temp
            ...
            mult[0][0] = mult[0][0] % 26 #ORIGINAL: 100
            mult[1][0] = mult[1][0] % 26 #ORIGINAL: 100
            mult[2][0] = mult[2][0] % 26 #ORIGINAL: 100
    ...
    if iter > 0: #Exited loop with partially or fully filled block of three values. Do padding where necessary with space character.
        if iter == 1:
            ...
            temp[1][0] = charIndex("a") #ORIGINAL: " "
            temp[2][0] = charIndex("a") #ORIGINAL: " "
            mult = np.matmul(key,temp) #3x3 * 3x1 = 3x1
            mult[0][0] = mult[0][0] % 26 #ORIGINAL: 100
            mult[1][0] = mult[1][0] % 26 #ORIGINAL: 100
            mult[2][0] = mult[2][0] % 26 #ORIGINAL: 100
